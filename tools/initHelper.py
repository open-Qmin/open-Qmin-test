""" Create initialConfigurationFile from NumPy functions """
...
def create_init_state(
    ...
    whole_Lx, whole_Ly, whole_Lz = dims_whole 
    n_sites_whole = whole_Lx * whole_Ly * whole_Lz
    state_array = np.empty((n_sites_whole, 10)) # will hold saved data
    Z, Y, X = np.meshgrid(np.arange(whole_Lz), np.arange(whole_Ly), np.arange(whole_Lx))
    # coords in first three columns
    state_array[...,0:3] = np.array([X.ravel(), Y.ravel(), Z.ravel()]).T 

    # create director array
    director_array = np.empty((n_sites_whole, 3))
        ...
        for Ry in range(ranks_y):
            for Rx in range(ranks_x): 
                ...
                state_array_chunk = state_array[
                    ...
                    # (Rx-1)*Lx:Rx*Lx, (Ry-1)*Ly:Ry*Ly, (Rz-1)*Lz:Rz*Lz].reshape(-1, 10)                                                                
